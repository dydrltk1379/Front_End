...
public class ForwardServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 파라미터를 받아  JoinMemberServlet 이동하는 코드

        // 1. 다른 Servlet으로 전달할 데이터를 req에 담아 전달하는 방법
        // req.setAttribute() : key-value를 통해 추가적인 정보를 담는 공간 (request scope) 기능

        req.setAttribute("MSG", "안녕하세요? 메시지입니다.");
        req.setAttribute("MSG_DATE", new Date());
        req.setAttribute("MSG_LIST", new ArrayList<>(Arrays.asList(1,2,3,4,5))); // 리스트로 전달하는 객체
        req.setAttribute("MSG", "덮어 쓴 메시지"); // 덮어쓰면 기존 데이터가 사라짐

        // 2. setAttribute 값 읽어오는 방법(다른 Servlet이나  JSP에서 읽는 것이 일반적)
        // req.getAttribute : 속성값을 읽어오는 메소드
        String MSG = (String) req.getAttribute("MSG");
        Date MSG_DATE = (Date) req.getAttribute("MSG_DATE");
        List MSG_LIST = (List) req.getAttribute("MSG_LIST");

        System.out.println(MSG);
        System.out.println(MSG_DATE);
        System.out.println(MSG_LIST);

        // 3.getParameter로도 접근할 수 있다.
        System.out.println("id : " + req.getParameter("id"));

        // 4. page를 forward로 이동하는 방법(url)
