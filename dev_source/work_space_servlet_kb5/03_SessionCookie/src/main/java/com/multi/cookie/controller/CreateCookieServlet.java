...
public class CreateCookieServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 2. 필요한 쿠키 설정을 추가한다. (아래 셋팅은 옵션이다(필수 아님))
        cookie1.setDomain("localhost"); // 쿠키의 유효 도메인
//        cookie2.setSecure(true); // HTTPS로만 전달받을 때 쿠키를 사용할 수 있다.
        cookie1.setPath(req.getContextPath()); // 쿠키의 유효 경로 설정.
        cookie1.setMaxAge(60*60*24); // 쿠키가 Client 브라우저에서 살아있는 시간
        cookie1.setMaxAge(60*2); // 2분
